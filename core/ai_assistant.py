...
COURSE_KNOWLEDGE = {
    ...
    "高檔爆量三類型": (
        "【高檔爆量三類型：調節量 vs 換手量 vs 出貨量 (CH4-3 朱家泓老師親授)】\n"
    ...
    "鎖第一波做第二波": (
        "【飆股主升段戰法：鎖第一波，做第二波 (CH5-5 朱家泓老師親授)】\n"
        "飆股絕不是隨機抓的，朱老師親授最強實戰心法：\n"
        "1. 鎖第一波：\n"
...
- 四大金剛循序看盤，任何股票的漲跌脈絡都能一目了然！"""
    ...
    if ("進場六問" in q) or ("六問" in q) or ("自我審查" in q) or ("進場前先問" in q):
        return f"""### 🧑‍🏫 【技術分析實戰助教 · CH5-6 朱家泓老師親授解答】
...
- 朱老師強調：**進場前一定要有冷靜的 SOP 檢查清單**。六個問題只要有一題答案是「否」或「猶豫不決」，代表勝率不足或風險過大，必須忍住不出手！
- **「進場前先想輸，再想贏！」** 只要把停損與風報比算清楚，在對的趨勢與位置出手，長期下來獲利是必然的結果！"""

    # 朱老師 CH5-3 親授：14大淘汰選股法
    if ("14大淘汰" in q) or ("十四大淘汰" in q) or ("淘汰選股" in q) or ("淘汰法" in q) or ("剔除清單" in q) or ("剔除條件" in q):
        return f"""### 🧑‍🏫 【技術分析實戰助教 · CH5-3 朱家泓老師親授解答】
> 🎯 **【核心疑難：選股如何快狠準？朱老師 14 大淘汰選股法為何？】**
...
- 透過負面剔除法，把 1800 檔股票瞬間縮小到幾檔真正純淨完美的飆股候選名單，安全又省時！"""
    ...
    # 朱老師 CH4-3 & CH4-4 親授：高檔爆量三類型與假突破
    if ("換手量" in q) or ("調節量" in q) or ("出貨量" in q) or ("高檔爆量" in q) or ("假突破" in q):
        return f"""### 🧑‍🏫 【技術分析實戰助教 · CH4-3 & CH4-4 朱家泓老師親授解答】
...
1. **為什麼第一波不用急著追？**
   ...
   - 朱老師親授：**「飆股有慣性，主力做飆股絕不會只做一波就走！」** 第一波連拉 15%~30% 剛好幫我們篩選出市場上最強勢的龍頭股。
2. **回檔洗盤守月線是天賜良機**：
...
def diagnose_stock_deeply(code: str, query: str = "", as_of_date: str = None, *args, **kwargs):
    ...
    if signals_dict.get('main_wave_2nd', False):
        pros.append("【朱老師 CH5-5 主升段第二波發動】第一波急漲連噴洗盤守穩月線，今日放量過昨高站上5MA，為第二波飆股主升段起漲點！")

    if signals_dict.get('is_turnover_success', False):
        pros.append("【朱老師 CH4-3 換手量成功】高檔爆大量黑K或變盤線後，3天內強勢長紅過高，籌碼洗淨換手完畢續噴！")

    if signals_dict.get('is_attack_vol', False):
        pros.append("【朱老師 CH4-2 5MA 攻擊量達標】今日量能達 5MA 基本量 1.25 倍以上，多方主力出量推升！")
    ...
    if signals_dict.get('is_false_breakout_dump', False):
        cons.append("【朱老師 CH4-4 假突破誘多出貨】突破長紅後 3 天內長黑摜破該長紅最低點！主力誘多倒貨陷阱，必跑！")

    if signals_dict.get('is_volume_price_divergence', False):
        cons.append("【朱老師 CH4-2 量價背離警示】價漲量急縮背離或高檔出量滯漲，多方動能匱乏，慎防拉回倒貨！")

    # 朱老師 CH5-3 14大淘汰檢核加入缺點清單
