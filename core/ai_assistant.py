...
COURSE_KNOWLEDGE = {
    ...
    "均線糾結與四線空排": (
        "【均線糾結突破 vs 均線糾結跌破 (CH3 3-5 朱家泓老師親授)】\n"
        "1. 均線糾結做多突破：\n"
    ...
    "做多在月線上與波段停利": (
        "【做多要在月線上 & 波段有三波做三波 (CH3 3-5 朱家泓老師親授)】\n"
    ...
    "做多與做空比較": (
        "【做多 vs 做空利潤比較與高低檔糾結哲學 (CH3 3-5 朱家泓老師親授)】\n"
...
2. **為什麼高檔很少糾結、底部常躺平幾個月？**
   ...
   - 底部散戶賠光不敢買、主力在暗中慢慢吃貨，所以在低檔躺平 2~3 個月非常常見。這也是為什麼「低檔均線糾結突破」爆發力最強！"""
    ...
    # 09/19 朱老師 CH3 3-5 親授：做多要在月線上 & 波段有三波做三波
...
2. **多頭徹底死亡的標準（老朱鐵律）**：
   ...
   - 此時絕不可再心存僥倖，做多部位必須全數清單撤離，絕不凹單！"""
    ...
    # 09/19 朱老師 CH3 3-5 親授：均線糾結與四線空排
...
def diagnose_stock_deeply(code: str, query: str = "", as_of_date: str = None, *args, **kwargs):
    ...
    if signals_dict.get('ma_squeeze_breakout', False):
        pros.append("【朱老師 CH3 3-5 均線糾結突破】四線（5/10/20/60MA）低檔高度糾結後放量長紅第一根突破！朱老師親授：糾結突破後面常有 2~3 倍大行情，次日若未漲停鎖死，開平或小漲可把握進場！")
    ...
    if signals_dict.get('ma20_death_break', False):
        cons.append("【朱老師月線鐵律】跌破月線已超過 3 天且月線向下彎，多頭徹底終結變空頭，依老朱紀律絕不可逆勢做多，持股應全數清倉！")

    if signals_dict.get('ma_squeeze_breakdown', False) or signals_dict.get('bearish_alignment_4ma', False):
        cons.append("【朱老師 CH3 3-5 四線空排警訊】均線糾結後長黑摜破，形成 5MA < 10MA < 20MA < 60MA 四線空排下彎（如講義波若威 3163）！空方慣性沉重，嚴禁做多！")
