...
朱家泓老師在實戰指導時特別強調：**絕不可把兩者孤立看待，而是要同時設定「雙重保險」**：
...
「一字底」是朱家泓老師《技術分析全攻略》中最具爆發力的主升段起漲型態，其核心特徵與確認要件如下：
...
朱家泓老師實戰操作五步驟是將所有技術指標化繁為簡的SOP量化進出場指南：
...
def answer_question(user_query: str, stock_context: dict = None, as_of_date: str = None) -> str:
    ...
    response_lines.append("### 🧑‍🏫 【朱家泓技術分析助教 · 實戰解答】")
    if diag['is_replay']:
